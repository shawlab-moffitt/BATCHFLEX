...
#' @param cluster_number Used by evaluation pca and evaluation cluster HE to select the number of kmeans generated clusters to display. If NULL is selected, a Silhouette generated cluster number is used..
...
#' @param large_list A BatchFLEX generated list used with the export function.
...
#' @param heatmap_annotation A vector of column names from the meta file to include in the heatmap annotation. Default is set to batch.1 and the variable of interest if nothing is provided.
#' @param heatmap_rownames A TRUE/FALSE statement to determine whether or not rownames will be included in the heatmap. Default is set to FALSE.
#' @param heatmap_colnames A TRUE/FALSE statement to determine whether or not colnames will be included in the heatmap. Default is set to FALSE.
...
#' @param cores A numeric input for the number of cores to use if parallel is set to TRUE. Default is set to 1. If none is chosen and parallelize is TRUE default is set to the number of evaluation methods chosen, if possible.
...
#' @param meta_files_included A logical to determine if the function should attempt to group a matrix and meta file together.
...
Batch_FLEX = function(Batch_FLEX_function = c("batch_correct", "batch_evaluate"),
  ...
  if (!all(Batch_FLEX_function %in% c("process_merge_files", "simulate_data", "merge_data", "preprocess_matrix", "batch_correct", "batch_evaluate", "BatchFLEX_export", "BatchFLEX_shiny"))){
  ...
  if ("process_merge_files" %in% Batch_FLEX_function){
    processed_merge_files <- process_merge_files(files_tobe_merged_path, meta_files_included, all_transnormed)
    merge_matrix_files <- processed_merge_files$matrix
    merge_meta_files <- processed_merge_files$meta
    Batch_FLEX_list$process_merge_data <- data.frame(processed_merge_files$individual_matrices, processed_merge_files$individual_meta_files)
    message("Matrices processed are ...")
    print(processed_merge_files$individual_matrices)
    message("Meta files processed are ...")
    print(processed_merge_files$individual_meta_files)
  }
  ...
  if ("batch_correct" %in% Batch_FLEX_function){
    ...
    Batch_FLEX_list$data_matrices <- base::append(Batch_FLEX_list$data_matrices, batch_correct(mat, meta, correction_method, batch.1, batch.2, log2_transformed, variable_of_interest, housekeeping,
                                          k, drop, center, round, tolerance, par.prior, sva_nsv_method, cores, parallelize))
  }
  if (!"batch_correct" %in% Batch_FLEX_function){
    ...
    for (name in names(Batch_FLEX_list$data_matrices)){
      if(is.null(batch.2)){
        if (grepl("Unadjusted", name)){
          ...
          plot_titles[[name]] <- paste0(name, "\n", "Batch_1: ", batch.1, "\n", "VOI: ", if (is.null(variable_of_interest)) {"NULL"} else {variable_of_interest}, sep = "")
        }
      } else {
        if (grepl("Unadjusted", name)){
          plot_titles[[name]] <- name
  ...
  if ("batch_evaluate" %in% Batch_FLEX_function){
    if (parallelize == TRUE){
      message("Starting parallelized batch evaluation")
      if (cores == 1){
  ...
  if ("BatchFLEX_shiny" %in% Batch_FLEX_function){
